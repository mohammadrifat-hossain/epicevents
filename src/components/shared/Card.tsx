...
const Card = ({event, hasOrderLink, hidePrice}: CardProps) => {
  ...
  return (
    <div className="group relative flex min-h-[380px] w-full max-w-[400px] flex-col overflow-hidden rounded-xl bg-white shadow-sm transition-all hover:shadow-lg md:min-h-[438px] ">
      <Link href={`/events/${event._id}`} style={{backgroundImage: `url(${event.imageUrl})`}} className="flex-center flex-row bg-gray-50 bg-cover bg-center text-gray-500 h-full"/>
      {/* IS EVENT CREATOR */}
      ...
      <div className="flex min-h-[230px] flex-col gap-3 p-5 md:gap-4">
        {!hidePrice && <div className="flex gap-2 ">
          ...
          <p className="p-semibold-14 w-min rounded-full bg-gray-500/10 px-4 py-1 text-gray-500">{event.category?.name ? event.category?.name : "Event"}</p>
        </div>}
